<?php 
	...
	if (empty($_POST['username']) || empty($_POST['text']) || empty($_POST['img'])) {
		...
		header("location:index.php?method=update");
	}
	$id = $_GET['id'];
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    	...
    	if (!move_uploaded_file($filename, $destination)) {
    		...
    		$sql = "update candidate set id='".$id."',name='".$username."',info='".$info."',pic='".$destination."',date='".$date."' where id='".$id."' ";
    		mysql_query($sql) or die("修改数据失败".mysql_error()); 
